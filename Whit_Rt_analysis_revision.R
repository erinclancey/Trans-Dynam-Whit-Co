...
J_date <- seq(230,362,1)

Whit_Co_weekly <- read.csv("Whit_CO_week.csv", header=TRUE)
Whit_Co_weekly <- subset(Whit_Co_weekly)
...
D <- ggplot(agg_R_Whit_df , aes(x=J_date, y=`Mean(R)`)) + theme_minimal()+
  ...
  scale_x_continuous(limits=c(237,365), n.breaks=9)+
...
PUL_R_t <- estimate_R(incid = Whit_Co_weekly$PUL_cases,
                      ...
                      config = make_config(list(
                        mean_si = GI_Whit, 
                        std_si = sd_GI_Whit)))
PUL_R_t_df <- PUL_R_t$R[-c(1:43),]
PUL_R_t_df$J_date <- J_date[-c(1:56)]
...
G <- ggplot()+
  ...
  scale_x_continuous(limits=c(240,365), n.breaks=9)+
  scale_y_continuous(limits=c(0,8), n.breaks=6)+
